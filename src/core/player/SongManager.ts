...
class SongManager {
  ...
  public prefetchNextSong = async (): Promise<AudioSource | undefined> => {
    try {
      ...
      if (officialUrl && !isTrial) {
        ...
        if (unlockUrl.url) {
          this.nextPrefetch = { ...unlockUrl, id: songId, isUnlocked: true };
          return this.nextPrefetch;
        } else if (officialUrl && settingStore.playSongDemo) {
          // 解锁失败，若官方为试听且允许试听，保留官方试听地址
          this.nextPrefetch = { id: songId, url: officialUrl, isTrial, quality, isUnlocked: false, source: "official" };
          return this.nextPrefetch;
        } else {
          return;
        }
  ...
  public getAudioSource = async (song: SongType, forceSource?: string): Promise<AudioSource> => {
    ...
    // 本地文件直接返回 (仅 Electron 环境)
    if (song.path && song.type !== "streaming" && isElectron) {
      // 检查本地文件是否存在
    ...
    if (
      ...
      this.nextPrefetch = undefined;
      const isOfficialTrialPrefetch =
        cachedSource.source === "official" &&
        cachedSource.isTrial &&
        song.type !== "radio" &&
        settingStore.useSongUnlock;
      if (!isOfficialTrialPrefetch) {
        console.log(`🚀 [${songId}] 使用预加载缓存播放`);
        return cachedSource;
      }
      console.log(`🧹 [${songId}] 跳过官方试听预载，改为实时尝试备用音源`);
    }
    ...
    try {
      ...
      // 官方完整音源可直接播放；官方试听必须先让备用音源接管
      if (officialUrl && !isTrial) {
        return { id: songId, url: officialUrl, quality, isUnlocked: false, source: "official" };
      }
      // 如果官方失败（或为试听），且未强制指定 auto (或者指定了 auto 但允许回退 - 即 Auto 模式)
      ...
      if (officialUrl && isTrial && settingStore.playSongDemo) {
        ...
        return { id: songId, url: officialUrl, quality, isUnlocked: false, isTrial, source: "official" };
      }
